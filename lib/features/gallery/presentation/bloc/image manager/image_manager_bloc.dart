...
class ImageManagerBloc extends Bloc<ImageManagerEvent, ImageManagerState> {
  ImageManagerBloc() : super(ImageManagerInitial()) {
    ...
    Future<List<AssetEntity>> fetchMedias({
      ...
      // List to hold fetched media items
    ...
    on<LoadAlbums>((event, emit) async {
      ...
      try {
        ...
        if (albums != []) {
          List<AssetEntity> medias = await fetchMedias(
            album: albums[0],
            page: 100,
          );
          medias = medias
              .where(
                (e) => e.type == AssetType.image || e.type == AssetType.video,
              )
              .toList();
          emit(
            ImageManagerSuccessState(
